<template>
	<div>
		...
		<div style="display: inline-block;width: 250px;margin: 10px 0 0 20px;">
			<h1>船队信息</h1>
			<el-table :data="reportList" :row-class-name='rowcolor' @row-click='fn'>
		...
		<div style="display: inline-block;width: 800px;margin: 10px 0 0 20px;
			...
			<div style="font-size: 14px;color: #606266;">
				...
				<el-row style='margin-top: 20px;'>
				  <el-col :span="6" >
					   <el-select  placeholder="请选择流程"  v-model='flowid' style='width: 150px;' @change='fn1'>
				...
				<el-row style='margin-top: 20px;'>
				  ...
				  <el-col :span="6">
					<el-button type='primary' :disabled="this.status!='未完成'"   @click='begin'>开始作业</el-button>   <!-- :disabled="startButtonFlag" -->
				  </el-col>
				  <el-col :span="6">			
				  ...
				  <el-col :span="6">
					<el-button type='success' :disabled="this.status!='作业中'"  @click='end'>结束作业</el-button>
				  </el-col>
				</el-row>
				<div class="boat">
					<div class="wrap">
...
// 				completeButtonFlag:false,
		...
		methods:{
			...
			begin(){
				...
				// 传递给后端的数据
				var data = {reportid:this.reportid,flowid:this.flowid,completetime:this.completetime};
				this.$axios.post(url,this.$qs.stringify(data),{
					headers: {
						'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'
					}
					...
					// res是后端的响应
					this.status='作业中'
					this.$message("亲,这次机器已经开始上班了")
					this.reportList = res.data
				})
			},
			end(){
				// 后端网址
				var url1 = this.baseUrl+"/produceReport/completeTask"
				// 传递给后端的数据
				var data = {reportid:this.reportid,flowid:this.flowid,startjobtime:this.startjobtime,capacity:this.capacity,completetime:this.completetime};
